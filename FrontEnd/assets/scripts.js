...
*/
function displayWorksOnHomepage(dataGallery, categoryID) {
    ...
    for (let i = 0; i < dataGallery.length; i++) {
        if(categoryID === null || categoryID === dataGallery[i].categoryId){
...
function getWorks (categoryID = null ) {
    ...
    fetch("http://localhost:5678/api/works")
        ...
        .then((dataGallery) => {
            displayWorksOnHomepage(dataGallery, categoryID)
            displayWorksOnModal(dataGallery, categoryID)})
        .catch((error)=> {
            console.error(error)
    })
}
...
/**
* Affichage des catégories sur le site
*/
function displayCategoriesOnHomepage(dataCategories){
    ...
    button_all.classList.add('button_all')
    ...
    for (let j = 0; j < dataCategories.length; j++) {
        ...
        button.addEventListener('click', () => {
            setTimeout(() => {
                getWorks(dataCategories[j].id)
            }, 1000)
        })
    }         
}
function test(cat){
    const exemple = cat.getAttribute('data-id')
    console.log(exemple)
}
...
function getCategories () {
    fetch("http://localhost:5678/api/categories")
        ...
        .then((dataCategories) => {
            ...
            displayCategoriesOnModalPhoto(dataCategories)})
        .catch((error)=> {
            console.error(error)
    })
}
...
/**
 * Initialisation de la modal pour la gestion des projets existants
 */
...
/**
 * Affichage des projets dans la modal projets
 */
const modal_gallery = document.getElementById('modal-gallery')

function displayWorksOnModal (dataGallery, categoryID) {
    ...
    modal_gallery.innerHTML = ''
        ...
        for (let i = 0; i < dataGallery.length; i++) {
            if(categoryID === null){
                ...
                articlemodal.getAttribute('data-id')
                articlemodal.innerHTML = `
                    <div class="image_gallery" data-id="${dataGallery[i].id}">
                        <img src="${dataGallery[i].imageUrl}" alt="${dataGallery[i].title}">
                ...
                const image_gallery = document.querySelectorAll('.image_gallery')
...
function deleteWorks (work) {
    const workID = work.getAttribute('data-id')
    ...
    .then(response => response.json())
    .then(response => {
        console.log(`Le projet ${workID} a bien été supprimé`)
        // Supprimer visuellement
        /*work.remove()*/
    })
    .catch(error => {console.error(error)
        // Aie problème lors de la suppression

    })
}

/**
 * Afficher les catégories dans la modal ajout photo
 */

function displayCategoriesOnModalPhoto (dataCategories) {
    category_image.innerHTML = ""

    for (let j = 0; j < dataCategories.length; j++) {
        const option = document.createElement('option')
        option.getAttribute('categories-id')
        option.innerHTML = `<option categories-id="${dataCategories[j].id}">${dataCategories[j].id}</option>`
...
btnvalidation.addEventListener('click', () => {
    ...
    const categories_image = document.getElementById('category_image').value;
    ...
    console.log(categories_image)
        ...
        .then((responseApisend) => responseApisend.json())
        .then((dataApisend) => {
            getWorks()
...
if (isEditMode()) {
    ...
    const btnbacktrack = document.querySelector('.btnbacktrack')
    btnbacktrack.addEventListener('click', () => {
        console.log("test")
    ...
    // ON crée le loader
    modal_gallery.innerHTML = '<p>Chargement en cours </p>'

    // Attend X millisecondes avant d'executer ce qu'il y a deadns
        ...
        .then((datamodal_Gallery) => {
            ...
            modal_gallery.innerHTML = '' // On reset le loader
            ...
            for (let i = 0; i < datamodal_Gallery.length; i++) {
                ...
                articlemodal.classList.add('modal-article')
                articlemodal.innerHTML = `
                <div class="image_gallery" data-id="${datamodal_Gallery[i].categoryid}">
                ...
                const image_delete = document.querySelector('.image_delete')
                const image_gallery = document.querySelectorAll('.image_gallery')
                ...
                image_gallery.forEach(item => {
                    ...
                    item.addEventListener('click', () => {
                        ...
                        // Faire le fetch et lui passer l'ID du work à supprimer
                        ...
                        .then((dataApi) => {
                            ...
                            // Si pas d'erreur => supprime le travail visuellement
                            // S'il y a une erreur retournée => faut afficher l'erreur
                            // Récupérer la div avec la classe modal-article qui à le data-id=${id}
                            // S'inspirer de ...getAttributes('data-id')
                            // ou alors de .querySelector('.data-article[data-id="${id}"]')
            ...
            .catch(err => {
                modal_gallery.innerHTML = '<p>Une erreur est survenue</p>'
            })*/
...
btnfile.addEventListener('change', () => {
    ...
    reader.onloadend = function () {
        ...
        modal_image.src = reader.result; // On remplace la source de l'image par le contenu du fichier en base 64
    }
 ...
 * 
 */
function getWorkssss(isJeSaisPasQuois){
    ...
    // fetch qui récupère les travaux
...
function openModals(){
    ...
    const works = getWorks(true); // récupérer les travaux avec le fetch

    displayWorksOnModals(works)


}

function displayWorksOnModals(works){
    // display les travaux dans les bonnes balises de la modals
}
